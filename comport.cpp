...
bool ComPort::checkPortForAvailable(QString port) {
    QList<QSerialPortInfo> portList = QSerialPortInfo::availablePorts();
    for(QSerialPortInfo portInfo : portList) {
        if(portInfo.portName() == port) return true;
    }
    return false;
}

void ComPort::setPortState(bool state) {
    QString portName = settings->getComPort();
    if(!checkPortForAvailable(portName)) return;
    ...
    if (state) {
        if (serialPort->open(QIODevice::ReadWrite)) {
            ...
            if (serialPort->setStopBits(stopBits) &&
                    ...
                    serialPort->setBaudRate(static_cast<qint32>(settings->getComBaudrate()))) {
            } else {
